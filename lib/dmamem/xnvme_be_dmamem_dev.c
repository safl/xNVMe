...
_rte_term(void)
{
	...
	if (g_dmamem_rte.ioas_alive) {
		iommufd_destroy(&g_dmamem_rte.iommufd, g_dmamem_rte.iommufd.ioas_id);
...
_rte_init_vfio_cdev(size_t heap_size)
{
	...
	err = iommufd_ioas_alloc(&g_dmamem_rte.iommufd);
	if (err) {
	...
	err = dmamem_from_memfd(&g_dmamem_rte.dmem, &g_dmamem_rte.iommufd, heap_size, hugepgsz);
	if (err) {
...
xnvme_be_dmamem_ctrlr_init(struct xnvme_dev *dev)
{
	...
	case XNVME_BE_DMAMEM_MODE_VFIO_CDEV:
		err = nvme_controller_open_dmamem_vfio(ctrlr->ctrl, &ctrlr->ctx,
						       &g_dmamem_rte.iommufd, &g_dmamem_rte.heap,
						       cdev_path);
		break;
	case XNVME_BE_DMAMEM_MODE_UIO_LUT:
