 ...
 * so the default of 0, meaning the full 48-bit user range, reserves 1 GiB with
 * 2 MiB hugepages. That is virtual and demand-paged, and unremarkable for this
 * class of software, but `ulimit -v` refuses it and `vm.overcommit_memory=2`
 * charges it against CommitLimit regardless of MAP_NORESERVE. This lets an
 * operator in either situation bound it; a region mapped above the bound then
