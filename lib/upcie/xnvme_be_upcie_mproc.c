...
xnvme_be_upcie_mproc_rte_init(int shm_id)
{
	...
	mode_t shm_mode;
	int lock_fd = -1, shm_fd = -1, oflag, shm_size, err;
	...
	lock_fd = open(mproc->lock_name, O_CREAT | O_RDWR, 0600);
	if (lock_fd < 0) {
		err = -errno;
	...
	// Wait at most ~1 second to open shared memory segtent. We do not retry in primary
	// processes.
