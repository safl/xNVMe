			    ...
			    uint64_t *phys)
{
	...
	err = dmamem_registry_add(&g_upcie_cuda_rte.registry, vaddr, nbytes, NULL);
	if (err) {
	...
	if (phys) {
		*phys = dmamem_va_to_iova(&g_upcie_cuda_rte.dmem, vaddr);
		if (!*phys) {
			...
			dmamem_registry_remove(&g_upcie_cuda_rte.registry, vaddr);
			return -EINVAL;
		}
	}
...
{
	return dmamem_registry_remove(&g_upcie_cuda_rte.registry, vaddr);
}
