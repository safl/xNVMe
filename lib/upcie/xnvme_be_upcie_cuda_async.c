...
/**
 * Queue setup for `upcie-cuda`: XNVME_QUEUE_CQ_MIRROR
 *
 ...
 * `upcie`'s unchanged. Only a controller this process opened can have it; a
 * served controller's queue is the server's to place.
 */
#include <libxnvme.h>
#include <errno.h>
...
_queue_init(struct xnvme_queue *queue, int opts)
{
	...
	if (g_upcie_rte.connection.alive) {
		...
		XNVME_DEBUG("FAILED: XNVME_QUEUE_CQ_MIRROR on a served controller");
		return -ENOTSUP;
	}
