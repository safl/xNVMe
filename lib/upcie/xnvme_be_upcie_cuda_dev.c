...
_cuda_rte_term(void)
{
	 ...
	 * in it. */
	xnvme_be_upcie_iova_range_close(&g_upcie_cuda_rte.iova_range);
		 ...
		 const char *bdf)
{
	...
	/* Where the controller consumes IOVAs, the heap has to be installed into
	 * this one's domain as well, at the addresses the table already holds. */
	if (g_upcie_cuda_rte.iova_range.alive) {
		int err = xnvme_be_upcie_iova_range_attach(&g_upcie_cuda_rte.iova_range, bdf);

		if (err) {
			XNVME_DEBUG("FAILED: iova_range_attach(%s); err(%d)", bdf, err);
			memset(slot, 0, sizeof(*slot));
			return err;
		}
	} else if (g_upcie_rte.connection.alive) {
	       ...
	       const char *bdf)
{
	...
	if (g_upcie_rte.connection.alive) {
		 ...
		 * table holds the IOVAs that came back. The range is claimed
		 * from the IOAS first, so what is handed out here cannot
		 * collide with what iommufd hands out later. */
		err = xnvme_be_upcie_iova_range_open(&g_upcie_cuda_rte.iova_range, bdf);
		if (!err) {
			err = dmamem_from_cuda_iommu_map_pa(
				&g_upcie_cuda_rte.dmem, &g_upcie_cuda_rte.cuda_heap,
				xnvme_be_upcie_va_bits(), g_upcie_cuda_rte.iova_range.imp);
		}
	} else {
		err = -ENOTSUP;
	}
	...
	if (!err && !g_upcie_rte.connection.alive) {
		...
		if (_cuda_doorbells_init(slot, bar->region, bar->size, bdf)) {
			XNVME_DEBUG("FAILED: no doorbell mapping the GPU can reach");
		}
	}
	if (err) {
		XNVME_DEBUG("FAILED: describing the CUDA heap to the controller; err(%d)", err);
		/* Before the heap goes, since closing the descriptor drops every
		 * mapping made on it. Nothing else will: the runtime is not
		 * marked initialised, so the teardown returns early. */
		xnvme_be_upcie_iova_range_close(&g_upcie_cuda_rte.iova_range);
		cudamem_heap_term(&g_upcie_cuda_rte.cuda_heap);
		cuCtxDestroy(g_upcie_cuda_rte.cu_ctx);
		return err;
