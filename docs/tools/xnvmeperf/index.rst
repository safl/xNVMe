...
With ``--be upcie-cuda`` the payloads land in GPU memory while the CPU drives
the queues. Adding ``--cq-mirror`` places the completion queues there as well, so
the controller's completion writes no longer queue behind its data writes; see
:ref:`sec-backends-upcie-cuda-cq-mirror` for when that matters. Example::

   xnvmeperf run --iopattern randread --qdepth 128 --iosize 512 \
       --runtime 10 --cpulist 0 --be upcie-cuda --cq-mirror 0000:01:00.0
