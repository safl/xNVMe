...
| CQ, with `XNVME_QUEUE_CQ_MIRROR` | GPU device memory, mirrored into the host CQ by a resident kernel | Keeps the controller's completion writes behind its data writes on one path; see {ref}`sec-backends-upcie-cuda-cq-mirror` |
...
(sec-backends-upcie-cuda-cq-mirror)=
...
Opening a queue with `XNVME_QUEUE_CQ_MIRROR` places its completion queue in device
...
last; it needs no stream or context from the caller. The `--cq-mirror` flag of
...
{ref}`sec-backends-upcie-cuda-cq-mirror`, is unaffected and works either way.
```
