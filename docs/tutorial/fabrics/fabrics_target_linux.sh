...
mkdir -p "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}"

echo "# Set subsystem access to 'attr_allow_any_host'"
echo 1 > "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}/attr_allow_any_host"

nsid=0
for dev_path in ${NVMET_DEVPATHS}; do
	nsid=$((nsid + 1))
	echo "# Create a NVMe Namespace within the Target Subsystem"
	mkdir -p "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}/namespaces/${nsid}"

	echo "# Export (${dev_path}) -- add device to kernel subsystem"
	echo -n "${dev_path}" > "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}/namespaces/${nsid}/device_path"

	# Enable the NVMe Target Namespace
	echo 1 > "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}/namespaces/${nsid}/enable"
done
...
ln -s "/sys/kernel/config/nvmet/subsystems/${NVMET_SUBNQN}" \
 "/sys/kernel/config/nvmet/ports/1/subsystems/${NVMET_SUBNQN}"
