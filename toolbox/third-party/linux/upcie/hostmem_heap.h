...
/**
 * A description of the heap, living at its own offset zero
 *
 ...
 * rather than repeating it, and it gives a consumer somewhere to find the
...
struct hostmem_heap_hdr {
	...
	uint64_t record_offset; ///< Set by the owner; zero when it has set nothing
...
hostmem_heap_hdr(const struct hostmem_heap *heap)
{
 ...
 * because the free list is a chain of addresses in the owner's mapping and has
...
/**
 * Point consumers at a record the owner placed in the heap
 *
...
{
	hostmem_heap_hdr(heap)->record_offset = offset;
}

/**
 * The offset the owner recorded, or zero when it recorded none
 *
...
{
	return hostmem_heap_hdr(heap)->record_offset;
}
